...
// sends out a request to the AX-12 servo to report its current position
{
	...
	{					// check if the right command is confirmed
		if ((int_busRxPkt.paramCnt == 0x02) && (int_busRxPkt.cmd == 0x00))
...
//	while (ServoAngle > 1) 	ServoAngle = GetServoAngle();
	...
	while(SetServoSpeed(0, CCW) == FALSE);					// stop motor
...
int main(void)
{
    ...
    timerAttach(TIMER2OVERFLOW_INT, flagOperation);
#endif
...
// -------------------------------------------------------------------------
	...
	GoalAngle = 2400;		// init variables used for servo mode control
	GoalSpeed = 1000;
	u08 Precision = 4;
	u08 Ramp = 10;
	u08 Jerk = 50;
...
//	while(1);
	...
	params[2] = 0x01;
	params[3] = 0xc1;
	cmd_BuildPacket(mySlaveClass, myAddress, 4, 0x12, params);
	cmd_Execute(INTERNAL_SERIAL);	

	SetServoSpeed(1023, CW);
